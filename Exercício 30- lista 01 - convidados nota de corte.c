...
int main(){
	...
	int qp, nc, p1,p2, count=1,convidado=0;
	printf("\nDigite a quantidade de participantes: ");
	scanf("%d", &qp);
	printf("\nDigite a nota de corte: ");
	scanf("%d", &nc);
	for(count=1;count<= qp; count++){
		printf("\nDigite a nota da primeira prova do %d° participante: ", count);
		scanf("%d", &p1);
		printf("\nDigite a nota da segunda prova do %d° participante: ", count);
		scanf("%d", &p2);
		if((p1 + p2) >= nc){
			convidado++;
		}
